...
class TestMyLab(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.obj =Sputnuk(self.name, self.mass, self.size)
    ...
    def test_instance(self):
        self.assertIsInstance(self.obj, Sputnuk)
    ...
    def test_mass_less_zero(self):
        with self.assertRaises(AssertionError):
            Sputnuk("Atlas V", -1, 58.3)

    def test_object_created(self):
        for name, mass, size in [("a", 1, 1), ("b", 2, 2)]:
            self.assertIsInstance(Sputnuk(name, mass, size), Sputnuk)
